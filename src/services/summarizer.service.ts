...
export class SummarizerService {
	...
	async summarize(text: string, config?: SummarizerConfig): Promise<SummaryResult> {
		const { maxLength = 200, style = 'concise', language = 'English' } = config || {};
	...
	private buildSystenPrompt(maxLength: number, style: string, language: string): string {
		const styleInstructructions: Record<SummaryStyles, string> = {
			concise: 'Create a brief, concise summary focusing on the main points.',
			detailed: 'Create a comprehensive summary that covers key details and context.',
			'bullet-points': 'Create a summary using bullet points to highlight key information.',
		};
