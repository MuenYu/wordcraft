...
export async function createTodoAction(formData: FormData) {
	try {
		...
		await db.insert(todos).values({
			...validatedData,
			userId: user.id,
			status: (validatedData.status as (typeof TodoStatus)[keyof typeof TodoStatus]) || TodoStatus.PENDING,
			priority: (validatedData.priority as (typeof TodoPriority)[keyof typeof TodoPriority]) || TodoPriority.MEDIUM,
