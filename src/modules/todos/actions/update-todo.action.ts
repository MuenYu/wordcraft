...
import { type TodoPriority, TodoStatus } from '@/modules/todos/models/todo.enum';
	...
	try {
		...
		const result = await db
			...
			.set({
				...
				...(priority && {
					priority: priority as (typeof TodoPriority)[keyof typeof TodoPriority],
				}),
...
export async function updateTodoFieldAction(todoId: number, data: { completed?: boolean }) {
	try {
		const user = await requireAuth();
		const db = await getDb();
